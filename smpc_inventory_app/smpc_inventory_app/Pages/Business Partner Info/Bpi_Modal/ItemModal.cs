    ...
    public partial class ItemModal : Form
    {
        ...
        private void dgv_itemList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                int item_id = int.Parse(dg_ItemList.Rows[e.RowIndex].Cells[0].Value.ToString());
                string short_desc = dg_ItemList.Rows[e.RowIndex].Cells[1].Value.ToString();
                string item_code = dg_ItemList.Rows[e.RowIndex].Cells[2].Value.ToString();
                string status_trade = dg_ItemList.Rows[e.RowIndex].Cells[6].Value.ToString();
                string status_tangible = dg_ItemList.Rows[e.RowIndex].Cells[7].Value.ToString();
