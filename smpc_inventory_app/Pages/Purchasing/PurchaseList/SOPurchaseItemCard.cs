...
{
    public partial class SOPurchaseItemCard : UserControl
    {
        public bool orderIsChecked => chk_so.Checked;
        ...
        public SOPurchaseItemCard(
            ...
            string quoteSupplier,
            string itemCode,
            string itemDescription,
            string unitOfMeasure,
            string itemName,
            string itemBrand,
            string commitmentDates,
            //string orderQtys,
            string qtys,
            //string qty,
            string totalQty,
            DataTable dt)
        {
            ...
            this.SalesOrderNos = salesOrderNos;
            ...
            this.CommitmentDates = commitmentDates;
            this.OrderQtys = qtys;
            this.Qtys = qtys;
        ...
        private async void PurchaseItemCard_Load(object sender, EventArgs e)
        {
        ...
        private async void FetchGuidingPrice()
        {
            ...
            {
                guidingPriceView.RowFilter = $"item_id = '{ItemId}'";

                if (guidingPriceView.Count > 0)
                {
                    var lastPrice = guidingPriceView[0]["last_price"];
                    this.LastPurchasePrice = lastPrice.ToString();
                }
        ...
        private async void LoadPaymentTerms()
        {
        ...
        public void GetLowestSellingPrice()
        {
            string[] unitPrices = UnitPrices.Split(',');

            decimal lowestSellingPrice = decimal.MaxValue;

            int length = unitPrices.Length;

            for (int i = 0; i < length; i++)
            {
                string priceStr = i < unitPrices.Length ? unitPrices[i] : "0";

                if (decimal.TryParse(priceStr, out decimal price))
                {
                    decimal sellingPrice = price;

                    if (sellingPrice < lowestSellingPrice)
                    {
                        lowestSellingPrice = sellingPrice;
                    }
                }
            }

            this.LowestSellingPrice = lowestSellingPrice.ToString();
        }
        private async void SaveEditedCell(int rowIndex, int colIndex)
        {
            try
            {
                ...
                if (fieldName == "new_list_price" || fieldName == "discount")
                {
                    ...
                    {
                        // pass to previous before changing
        ...
        {
            string[] orderNos = SalesOrderNos.Split(',');
            string[] orderDetailIds = OrderDetailIds.Split(',');
            string[] qtys = Qtys.Split(',');
            string[] unitPrices = UnitPrices.Split(',');
            string[] discounts = Discounts.Split(',');
            string[] commitmentDates = CommitmentDates.Split(',');
            string[] quoteSuppliers = QuoteSupplier.Split(',');
            ...
            dt.Columns.Add("quote_supplier", typeof(string));

            int maxLength = new[] { orderNos.Length, orderDetailIds.Length, qtys.Length, unitPrices.Length, discounts.Length, commitmentDates.Length, quoteSuppliers.Length }.Max();
            ...
            for (int i = 0; i < maxLength; i++)
            {
                ...
                string unitPrice = i < unitPrices.Length ? unitPrices[i] : "0";
                if (decimal.TryParse(unitPrice, out decimal priceValue))
                {
                    unitPrice = priceValue.ToString("C", new CultureInfo("en-PH"));
                }
                else
                {
                    unitPrice = "N/A";
                }
                string discount = i < discounts.Length ? discounts[i] : "0";
                decimal discountVal = decimal.TryParse(discount, out decimal parsedDiscount) ? parsedDiscount : 0;
                decimal sellingPriceValue = priceValue - (priceValue * discountVal / 100);
                string sellingPrice = sellingPriceValue.ToString("C", new CultureInfo("en-PH"));
                string commitmentDate = i < commitmentDates.Length ? commitmentDates[i] : "N/A";
                string quoteSupplier = i < quoteSuppliers.Length ? quoteSuppliers[i] : "N/A";

                dt.Rows.Add(orderNo, orderDetailId, qty, unitPrice, discount, sellingPrice,  commitmentDate, quoteSupplier);
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txt_order_qty.Text) || txt_order_qty.Text == "0")
            {
                ...
                chk_so.CheckedChanged -= checkBox1_CheckedChanged;
        ...
        private async void dgv_canvass_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                if (dgv_canvass.Columns[e.ColumnIndex].Name == "supplier_name" && dgv_canvass.Rows[e.RowIndex].IsNewRow)
                {
                    ...
                    SupplierModal modal = new SupplierModal(existingSupplierIds, this.ItemId);
        ...
        {
            this.OrderQty = txt_order_qty.Text;
            Console.WriteLine("New order quantity: " + Qtys);
        }
        // HELPERS
        ...
        private void dgv_guiding_price_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void chk_so_EnabledChanged(object sender, EventArgs e)
        {
            bool currentState = chk_so.Enabled;
            Console.WriteLine("Enabled changed: " + currentState);
        }

        private void dgv_canvass_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            var dgv = sender as DataGridView;
            var cell = dgv.Rows[e.RowIndex].Cells[e.ColumnIndex];

            // Try parse if the column expects decimal
        ...
        private void dgv_canvass_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
